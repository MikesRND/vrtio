...
TEST(EndianTest, ByteSwap16) {
    ...
    uint16_t swapped = vrtio::detail::byteswap16(value);
    EXPECT_EQ(swapped, 0x3412);

    // Double swap should return original
    EXPECT_EQ(vrtio::detail::byteswap16(swapped), value);
}

TEST(EndianTest, ByteSwap32) {
    uint32_t value = 0x12345678;
    uint32_t swapped = vrtio::detail::byteswap32(value);
    EXPECT_EQ(swapped, 0x78563412);

    // Double swap should return original
    EXPECT_EQ(vrtio::detail::byteswap32(swapped), value);
}

TEST(EndianTest, ByteSwap64) {
    uint64_t value = 0x123456789ABCDEF0ULL;
    uint64_t swapped = vrtio::detail::byteswap64(value);
    EXPECT_EQ(swapped, 0xF0DEBC9A78563412ULL);

    // Double swap should return original
    EXPECT_EQ(vrtio::detail::byteswap64(swapped), value);
}

// Test network byte order conversions
TEST(EndianTest, HostToNetwork32) {
    uint32_t value = 0xDEADBEEF;
    uint32_t network = vrtio::detail::host_to_network32(value);
    uint32_t host = vrtio::detail::network_to_host32(network);
...
TEST(EndianTest, HostToNetwork64) {
    ...
    uint64_t network = vrtio::detail::host_to_network64(value);
    uint64_t host = vrtio::detail::network_to_host64(network);
...
TEST(EndianTest, ConstexprFunctions) {
    ...
    constexpr uint32_t swapped = vrtio::detail::byteswap32(0x12345678);
    constexpr uint32_t network = vrtio::detail::host_to_network32(0xDEADBEEF);
...
TEST(EndianTest, PlatformDetection) {
    ...
    EXPECT_TRUE(vrtio::detail::is_little_endian || vrtio::detail::is_big_endian);
...
TEST(EndianTest, EdgeCases) {
    ...
    EXPECT_EQ(vrtio::detail::byteswap16(0), 0);
    EXPECT_EQ(vrtio::detail::byteswap32(0), 0);
    EXPECT_EQ(vrtio::detail::byteswap64(0), 0);

    // Max values
    EXPECT_EQ(vrtio::detail::byteswap16(0xFFFF), 0xFFFF);
    EXPECT_EQ(vrtio::detail::byteswap32(0xFFFFFFFF), 0xFFFFFFFF);
    EXPECT_EQ(vrtio::detail::byteswap64(0xFFFFFFFFFFFFFFFFULL), 0xFFFFFFFFFFFFFFFFULL);

    // Powers of 2
    EXPECT_EQ(vrtio::detail::byteswap32(0x00000001), 0x01000000);
    EXPECT_EQ(vrtio::detail::byteswap32(0x80000000), 0x00000080);
}
...
TEST(EndianTest, VRTFieldConversion) {
    ...
    uint32_t network_stream = vrtio::detail::host_to_network32(stream_id);
    ...
    uint32_t recovered = vrtio::detail::network_to_host32(read_network);
...
TEST(EndianTest, TimestampConversion) {
    ...
    uint64_t recovered = vrtio::detail::network_to_host64(read_network);
