...
/**
 * @brief Zero-allocation VRT file reader with templated scratch buffer
 *
 * Reads VRT packets from binary files with multiple API patterns:
...
public:
    ...
    VRTFileReader& operator=(VRTFileReader&& other) noexcept {
        if (this != &other) {
            if (file_) {
                std::fclose(file_);
            }
